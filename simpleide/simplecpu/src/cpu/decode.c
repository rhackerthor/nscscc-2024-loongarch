...
/* memory */
#define Mread(vaddr) pmem_read(vaddr)
...
#define INSTPAT_MATCH(name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0010100000??????????????????????", ld_b      , SI12 , trace.rf_we = 1, trace.rf_waddr = rd, trace.rf_wdata = SEXT(BITS(Mread(src1 + imm), 7, 0), 8));
  INSTPAT("0010100010??????????????????????", ld_w      , SI12 , trace.rf_we = 1, trace.rf_waddr = rd, trace.rf_wdata = Mread(src1 + imm));

  INSTPAT("0010100100??????????????????????", st_b      , SI12 , trace.rf_we = 0, trace.rf_waddr =  0, trace.rf_wdata = 0, Mwrite(src1 + imm, cpu_gpr(rd), 1 << BITS(src1 + imm, 1, 0)));
  ...
  INSTPAT("011001??????????????????????????", bge       , SI16 , trace.rf_we = 0, trace.rf_waddr =  0, trace.rf_wdata = 0, cpu_next_pc = (src1 >= cpu_gpr(rd)) ? cpu_pc + imm : cpu_next_pc);
